...
const login = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(400)
        .send({ message: "1 Please check your email and password" });
    ...
    if (!match)
      return res
        .status(400)
        .send({ message: "2 Please check your email and password" });
    ...
    return res
      .status(500)
      .send({ message: "3 Sorry for inconvenience please try again later" });
  }
};
